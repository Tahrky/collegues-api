...
public class StartupDataInit {
	...
	public void init() {
		...
		utilisateurRepository.save(new UtilisateurSession(collegueRepo.findById(matriculeTemp).orElseThrow(Error::new),  passwordEncoder.encode("pass"), Arrays.asList("ROLE_USER")));
		...
		collegueRepo.save(new Collegue("1", "Bobbi", "Albert", "aa@a.a",
				...
				date1));
		utilisateurRepository.save(new UtilisateurSession(collegueRepo.findById("1").orElseThrow(Error::new), passwordEncoder.encode("pass"), Arrays.asList("ROLE_ADMIN", "ROLE_USER")));

		collegueRepo.save(new Collegue("2", "Bobbi", "Mylène", "ab@a.a", "https://tinyurl.com/y5dj758o", date1));
		utilisateurRepository.save(new UtilisateurSession(collegueRepo.findById("2").orElseThrow(Error::new),  passwordEncoder.encode("pass"), Arrays.asList("ROLE_USER")));
		...
		collegueRepo.save(new Collegue("3", "Paul", "Gurpratap Singh", "god@himself.universe",
				...
				date1));
		utilisateurRepository.save(new UtilisateurSession(collegueRepo.findById("3").orElseThrow(Error::new),  passwordEncoder.encode("pass"), Arrays.asList("ROLE_USER")));
		...
		collegueRepo.save(new Collegue("4", "Paul", "Petit", "small@paul.india",
				...
				date1));
		utilisateurRepository.save(new UtilisateurSession(collegueRepo.findById("4").orElseThrow(Error::new),  passwordEncoder.encode("pass"), Arrays.asList("ROLE_USER")));
		...
		matriculeTemp = UUID.randomUUID().toString();
		collegueRepo.save(new Collegue(matriculeTemp, "Jeltsch", "Julie", "julie@a.a",
